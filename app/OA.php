...
class OA
{
	...
	public function Sync()
	{
		...
		foreach($worklogs_submitted as $id=>$sworklog)
		{
			 echo $id;

			 foreach($sworklog as $date=>$wl)
			 {
				 		if(array_key_exists($id, $worklogs_approved))
						{
								if(array_key_exists($date, $worklogs_approved[$id]))
								{
									    $worklogs_approved[$id][$date]['decimal_hours'] += $wl['decimal_hours'];
											$worklogs_approved[$id][$date]['approved'] =  false;
								}
								else {
									// code...
									$worklogs_approved[$id][$date] =  $wl;
								}
						}
						else
						{
